...
def wavelet_reconstructor(signal, wavelet_name, decomposition_level):
    ...
    wavfile.write(f'{path}/recon-ca3.wav', samplerate, reconstructed_signal['cA3'])
    wavfile.write(f'{path}/recon-cd3.wav', samplerate, reconstructed_signal['cd3'])
    wavfile.write(f'{path}/recon-cd2.wav', samplerate, reconstructed_signal['cd2'])
    wavfile.write(f'{path}/recon-cd1.wav', samplerate, reconstructed_signal['cd1'])
    ...
    plt.savefig('../result/dwt-recon-plot.png', dpi=100)  # Saving plot as PNG image
    # plt.show()

    return reconstructed_signal

path = '../data/sound'
samplerate, data = wavfile.read(f'{path}/music.wav');  # Reading the audio file
