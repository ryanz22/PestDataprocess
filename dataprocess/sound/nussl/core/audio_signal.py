...
class AudioSignal(object):
    ...
    **Sample Rate**
    ...
    def stft_params(self, value):
        ...
        check_COLA(
            window_type, self._stft_params.window_length, self._stft_params.hop_length
        )
    ...
    def log_magnitude_spectrogram_data(self):
        ...
        if self.stft_data is None:
            raise AudioSignalException(
                "Cannot calculate log_magnitude_spectrogram_data "
                "because self.stft_data is None"
            )
    ...
    def get_window(window_type, window_length):
        ...
        if window_type == constants.WINDOW_SQRT_HANN:
            window = np.sqrt(scipy.signal.get_window("hann", window_length))
        else:
            window = scipy.signal.get_window(window_type, window_length)

        return window

    def stft(
        self, window_length=None, hop_length=None, window_type=None, overwrite=True
    ):
        """
    ...
    def ipd_ild_features(self, ch_one=0, ch_two=1):
        ...
        if self.stft_data is None:
            raise AudioSignalException(
                "Cannot compute ipd/ild features without stft_data!"
            )
        if self.is_mono:
    ...
    def rms(self, win_len=None, hop_len=None):
        ...
        if win_len is not None:
            ...
            rms_func = lambda arr: librosa.feature.rms(
                arr, frame_length=win_len, hop_length=hop_len
            )[0, :]
        else:
    ...
    def get_channel(self, n):
        ...
        return np.asfortranarray(np.take(self.audio_data, constants.CHAN_INDEX, n))
    ...
    def _apply_user_ordered_effects(self):
        ...
        while i < len(self._effects_chain):
            ...
            if j == len(self._effects_chain) or type(self._effects_chain[i]) != type(
                self._effects_chain[j]
            ):  # new fx type
    ...
    def make_effect(self, effect, **kwargs):
        ...
        try:
            ...
            raise AudioSignalException(
                f"Error calling {effect} with parameters {kwargs}: `{e}`"
            )
    ...
    def low_pass(self, freq, poles=2, width_type="h", width=0.707, **kwargs):
        ...
        See Also:
            ...
            * :func:`reset_effects_chain`: Empties the effects chain without applying any effects.
        """
        self._effects_chain.append(
            effects.low_pass(
                freq, poles=poles, width_type=width_type, width=width, **kwargs
            )
        )
        return self
    ...
    def chorus(
        self, delays, decays, speeds, depths, in_gain=0.4, out_gain=0.4, **kwargs
    ):
        """
    ...
    def emphasis(self, level_in, level_out, type_="col", mode="production", **kwargs):
        ...
        """
        self._effects_chain.append(
            effects.emphasis(level_in, level_out, type_=type_, mode=mode, **kwargs)
        )
        return self
